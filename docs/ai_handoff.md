...
### 14. 真新数据终极检验（阶段 20，已完成）

代码：`src/quant_factor/out_of_time.py`。把策略锁死（行业中性 + 阶段 17-19 的因子和符号），
拉取研究期之外的 2024-2025 真实行情检验——唯一 100% 干净的考场。独立运行（需联网）：
`uv run python -m quant_factor.out_of_time`。输出 `out_of_time_comparison.csv`。

核心结论（2024-2025）：锁死策略收益 +49.4% 略赢 SPY(+47.0%) 和等权池(+41.6%)，但 Sharpe
（1.235）略输给两个基准、回撤略大。本质"大致等于市场，无明显 alpha，但也没崩"。整条路的规律：
样本内很强 → 泄漏样本外赢等权 → 真新数据回落到市场水平。这是纯价量因子策略的真实归宿。
详见 `docs/stage_reports/stage20_out_of_time.md`。
...
### 推荐下一步：换更好的原料（阶段 20 的诚实结论指向这里）

阶段 20 的终极检验已经把话说清楚：**纯价量因子策略大致等于市场、无明显 alpha。**
所以继续调价量因子参数意义不大，方向应转向"原料"：

- **修幸存者偏差**：接入历史指数成分股，别再用今天的幸存者回测过去。
- **加价量之外的因子**：基本面、盈利质量、分析师预期、事件类等——新的信息维度才可能有真 alpha。
- **若上机器学习**：清醒地认识到，特征若仍是这几个价量因子，模型大概率只会学出同样的市场水平结果；
  ML 的价值在于更好的特征 + 更多数据，而非在弱特征上堆模型复杂度。
- 把"会证伪自己、用真新数据做终极检验"本身作为项目核心亮点（面试比吹高回测强得多）。
