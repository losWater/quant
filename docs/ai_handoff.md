...
### 推荐下一步：阶段 19，多因子中性版的滚动验证 + 初步风控

阶段 18 多因子已完成，样本外首次跑赢等权池，但只有一个窗口。下一步：

- **滚动样本外**：把多因子综合分接进阶段 16 的滚动框架（`build_rolling_neutral_comparison`
  已支持 sector_neutral，可扩展为对综合分逐年验证），确认"跑赢等权池"不是单窗口偶然。
- **初步风控**：多因子回撤变大（-0.387），尝试单票权重上限 / 波动率目标 / 最大回撤控制，
  看能否保住收益的同时压回撤。每次都要保留 SPY 和等权池对比、跑固定 + 滚动样本外。
- 在这个 baseline 稳定后，才进入最基础的机器学习：因子作为特征，让模型学组合权重
  （替代当前手工等权），但要用同样的样本外纪律检验。
